...
bufferInfo = image.getSourceInformation()
...
for f in sys.argv[1:]:
	...
	for word in src:
		...
		if word[:2] == "::":							# Magenta (variable)  	$83
			...
			tag = 0x86
			word = word[1:-1]
		elif word[0] == "{" and word[-1] == "}": 		# Cyan (compile macro)	$87
			tag = 0x85
			word = word[1:-1]
		#
		...
		image.write(currentPageNumber,currentPageAddress,len(word))
		currentPageAddress += 1
		for c in word:
			image.write(currentPageNumber,currentPageAddress,ord(c))
			currentPageAddress += 1
		bytesRemaining = bytesRemaining - 2 - len(word)
		image.write(currentPageNumber,currentPageAddress,0x80)
